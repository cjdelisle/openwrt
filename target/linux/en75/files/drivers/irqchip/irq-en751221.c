 ...
 * "shadow interrupt" because it is never fired and only exists for the purpose
 * of controlling the second VPE's view of its parent interrupt.
 *
 ...
 * example IRQ 31 shadows IRQ 30, put in your DTS file:
 *
 * econet,shadow-interrupts = <31 30>;
 *
 * Then your driver requiring a per-cpu interrupt should request IRQ 30 as it
 * normally would. You can put as many pairs as you want, e.g. <31 30 33 32>.
 *
...
static struct {
	...
	.shadow_interrupts = {
		[0 ... EN75_INTC_IRQ_COUNT - 1] = INTC_NO_SHADOW,
	},
};
