 ...
 * 0x00 which is used to tag factory blocks. Vender firmware does not do this,
 * so if the bootloader or vender OS marks a block bad, it will be
...
/* Size field is a u8 but vender firmware checksums over all 1000 places. */
#define MAX_BBT_SIZE				1000

/* Vender firmware calls this POOL_GOOD_BLOCK_PERCENT */
 ...
 * We sort the BBT so that we can do this in one pass. The vender firmware
			  ...
			  u16 bad_block_index)
{
	...
	/* vender firmware uses host order */
...
enum block_is_bad {
	/* Good block */
	BB_GOOD,
	/* Probably marked bad at the factory (or by vender firmware!) */
	BB_FACTORY_BAD,
	/* Marked bad by us */
	BB_WORN,
	/* We don't know */
...
static void r_reconstruct_bmt(struct en75_bmt_m *ctx)
{
	...
	/*
	 * The table must be zero because the vender firmware checksums
	 ...
	 * this number will increase on next boot!). But we and the vender
...
static int r_reconstruct_bbt(struct bbt_table *bbt_out, const struct en75_bmt_m *ctx)
{
	...
	/* Vender firmware checksums the entire table, no matter how much is used. */
...
static int try_parse_bmt(struct bmt_table *out, u8 *buf, int len)
{
	...
	/*
	 * The vender firmware checksums over rblocks entries, but zero
	 ...
	 * header and if the vender firmware left some trash in the
...
static int r_scan_reserve(struct en75_bmt_m *ctx)
{
	...
	for (; cursor > 0; cursor--) {
		...
		if (ret || fdm_is_bad(fdm)) {
			...
			pr_info("%s: found BBT in blockx %d\n", log_pfx, cursor);
			print_hex_dump(KERN_INFO, "BBT: ",
				       DUMP_PREFIX_NONE, 32, 1,
				       data_buf, pg_size, false);
			bif.status = BS_BBT;
